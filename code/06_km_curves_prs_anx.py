#!/usr/bin/env python3
"""
Kaplan-Meier Survival Analysis for Anxiety based on PRS, stratified by baseline diagnosis
...
prs_data = pd.read_csv('temp/npi_anx_ever.prs.best', sep=r'\s+')
print(f"PRS data shape: {prs_data.shape}")
print(f"PRS columns: {prs_data.columns.tolist()}")

# Load phenotype data
print("\nLoading phenotype data...")
pheno_data = pd.read_csv('temp/npi_anx_ever.csv')
...
print("\nMissing values:")
print(merged_data[['timepoint_censored', 'NPI_ANX_EVER', 'PRS', 'DX.bl']].isnull().sum())

# Remove rows with missing values in key variables
merged_data = merged_data.dropna(subset=['timepoint_censored', 'NPI_ANX_EVER', 'PRS', 'DX.bl'])
    ...
    for i, prs_group in enumerate(['Low PRS', 'High PRS']):
        ...
        if mask.sum() > 0:
            kmf.fit(
                ...
                event_observed=merged_data.loc[mask, 'NPI_ANX_EVER'],
...
ax.set_ylabel('Anxiety-Free Survival Probability', fontsize=13)
ax.set_title('Kaplan-Meier Curves: Anxiety Risk by PRS and Baseline Diagnosis', 
             fontsize=15, fontweight='bold', pad=20)
ax.legend(loc='best', fontsize=10, framealpha=0.9)
ax.grid(True, alpha=0.3)
plt.tight_layout()

output_file_combined = 'report/km_curve_npi_anx_ever_prs_stratified.png'
...
for idx, dx in enumerate(dx_order):
    ...
    if mask_low.sum() > 0:
        kmf_low.fit(
            ...
            event_observed=merged_data.loc[mask_low, 'NPI_ANX_EVER'],
            label='Low PRS'
        )
    ...
    if mask_high.sum() > 0:
        kmf_high.fit(
            ...
            event_observed=merged_data.loc[mask_high, 'NPI_ANX_EVER'],
            label='High PRS'
        )
    ...
    if mask_low.sum() > 0 and mask_high.sum() > 0:
        lr_result = logrank_test(
            ...
            event_observed_A=merged_data.loc[mask_low, 'NPI_ANX_EVER'],
            event_observed_B=merged_data.loc[mask_high, 'NPI_ANX_EVER']
        )
    ...
    if idx == 0:
        ax.set_ylabel('Anxiety-Free Survival Probability', fontsize=12)
...
output_file_separate = 'report/km_curve_npi_anx_ever_prs_by_diagnosis.png'
...
overall_lr = logrank_test(
    ...
    event_observed_A=merged_data.loc[low_prs_mask, 'NPI_ANX_EVER'],
    event_observed_B=merged_data.loc[high_prs_mask, 'NPI_ANX_EVER']
)
print(overall_lr)
...
for dx in dx_order:
    ...
    if mask_low.sum() > 0 and mask_high.sum() > 0:
        ...
        print(f"\n{dx}:")
        print(f"  Low PRS:  n={mask_low.sum()}, events={merged_data.loc[mask_low, 'NPI_ANX_EVER'].sum()}")
        print(f"  High PRS: n={mask_high.sum()}, events={merged_data.loc[mask_high, 'NPI_ANX_EVER'].sum()}")
...
mv_lr = multivariate_logrank_test(
    ...
    event_observed=merged_data['NPI_ANX_EVER']
)
print(mv_lr)
...
# Save Summary
# =============================================================================
summary_file = 'report/km_summary_npi_anx_ever_prs_stratified.txt'
with open(summary_file, 'w') as f:
    f.write("Kaplan-Meier Analysis Summary: Anxiety Risk by PRS (Stratified by DX.bl)\n")
    f.write("="*70 + "\n\n")
